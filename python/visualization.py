...
r_filt = dsp.ExpFilter(
    np.tile(0.01, config.N_PIXELS // 2), alpha_decay=0.2, alpha_rise=0.99
)
g_filt = dsp.ExpFilter(
    np.tile(0.01, config.N_PIXELS // 2), alpha_decay=0.05, alpha_rise=0.3
)
b_filt = dsp.ExpFilter(
    np.tile(0.01, config.N_PIXELS // 2), alpha_decay=0.1, alpha_rise=0.5
)
common_mode = dsp.ExpFilter(
    np.tile(0.01, config.N_PIXELS // 2), alpha_decay=0.99, alpha_rise=0.01
)
real_common_mode = dsp.ExpFilter(
    np.tile(0.01, config.N_PIXELS), alpha_decay=0.99, alpha_rise=0.01
)
p_filt = dsp.ExpFilter(
    np.tile(1, (3, config.N_PIXELS // 2)), alpha_decay=0.1, alpha_rise=0.99
)
...
def visualize_real_spectrum(y):
    """Effect that maps the Mel filterbank frequencies onto the LED strip"""
    global _prev_spectrum
    y = np.copy(interpolate(y, config.N_PIXELS))
    real_common_mode.update(y)
    y = y[::-1]
    # diff = y - _prev_spectrum
    # _prev_spectrum = np.copy(y)
    # # Color channel mappings
    ...
    # g = np.concatenate((g[::-1], g))
    # b = np.concatenate((b[::-1], b))
    output = np.array([y, y*0.5, y*0.0]) * 255
    return output
...
if __name__ == "__main__":
    if config.USE_GUI:
        ...
        def energy_click(x):
            ...
            real_spectrum_label.setText("Real Spectrum", color=inactive_color)
        ...
        def real_spectrum_click(x):
            global visualization_effect
            visualization_effect = visualize_real_spectrum
            energy_label.setText("Energy", color=inactive_color)
            scroll_label.setText("Scroll", color=inactive_color)
            spectrum_label.setText("Spectrum", color=inactive_color)
            real_spectrum_label.setText("Real Spectrum", color=active_color)

        # Create effect "buttons" (labels with click event)
        energy_label = pg.LabelItem("Energy")
        scroll_label = pg.LabelItem("Scroll")
        spectrum_label = pg.LabelItem("Spectrum")
        real_spectrum_label = pg.LabelItem("Real Spectrum")
        energy_label.mousePressEvent = energy_click
        scroll_label.mousePressEvent = scroll_click
        spectrum_label.mousePressEvent = spectrum_click
        real_spectrum_label.mousePressEvent = real_spectrum_click
        real_spectrum_click(0)
        # Layout
        layout.nextRow()
        ...
        layout.addItem(real_spectrum_label)
    # Initialize LEDs
    led.update()
